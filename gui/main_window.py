...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.stat_analysis_tab = StatisticalAnalysisWidget()
        tab_widget.addTab(self.stat_analysis_tab, "Analisi Statistica")

        self.error_prop_tab = ErrorPropagationWidget()
        tab_widget.addTab(self.error_prop_tab, "Propagazione Errori")
        ...
        self.stat_analysis_tab.variableComputed.connect(self.error_prop_tab.add_variable_from_value)
    ...
    def save_project(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Salva Progetto", "", "Progetto JSON (*.json)")
        if not file_name:
            return
        if not file_name.endswith('.json'):
            file_name += '.json'
        project = self.data_manager.to_project_dict()
        project['labels'] = {
            'title': self.plot_widget.title_input.text(),
            'x_label': self.plot_widget.x_label_input.text(),
            'y_label': self.plot_widget.y_label_input.text(),
        }
        try:
            with open(file_name, 'w') as f:
                json.dump(project, f, indent=2)
            QMessageBox.information(self, "Successo", "Progetto salvato correttamente.")
        except Exception as e:
            QMessageBox.warning(self, "Errore", f"Errore nel salvataggio del progetto: {str(e)}")

    def load_project(self):
        file_name, _ = QFileDialog.getOpenFileName(self, "Carica Progetto", "", "Progetto JSON (*.json)")
        if not file_name:
            return
        try:
            with open(file_name, 'r') as f:
                project = json.load(f)
            self.data_manager.load_project_dict(project)

            labels = project.get('labels', {})
            self.plot_widget.title_input.setText(labels.get('title', ''))
            self.plot_widget.x_label_input.setText(labels.get('x_label', ''))
            self.plot_widget.y_label_input.setText(labels.get('y_label', ''))

            self.series_widget.update_series_list()
            self.plot_widget.update_plot()
        except Exception as e:
            QMessageBox.warning(self, "Errore", f"Errore nel caricamento del progetto: {str(e)}")
