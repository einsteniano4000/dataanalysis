...
class ErrorPropagationWidget(QWidget):
    ...
    def add_variable_from_value(self, name, value, error):
        try:
            self.error_propagation.add_variable(name, value, error)
            self.update_var_list()
        except ValueError as e:
    ...
    def calculate(self):
        try:
            ...
            output = f"Risultato: {format_value_with_error(result['result'], result['absolute_error'])}\n"
            output += f"Errore relativo: {round_to_significant_figures(result['relative_error'], 2):g}\n"
            output += f"Errore percentuale: {round_to_significant_figures(result['percentage_error'], 2):g}%"
